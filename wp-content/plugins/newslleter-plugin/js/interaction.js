...
//
        ...
        $.post("http://localhost/wp-content/plugins/news.core.php", $('#lead-action-form').serializeArray())
                .done(function (data) {
                    ...
                    var line = '<tr> <td style="background-color:#B0C4DE" class="lead-title-td"><span class="glyphicon glyphicon-cloud-download" aria-hidden="true"> </span> ' + resource[0].name + '</td>' +
                            ...
                            '<a class="lead-link-update" href="news.core.php?lead_value_id=' + resource[0].id + '">' +
    ...
    $(document).on('click', ".lead-link-update", function (event) {
        ...
        $.ajax(this.href, {success: function (data) {
                ...
                var cargo;
                if (resource[0].cargo === 1) {
                    cargo = "DBA";
                } else if (resource[0].cargo === 2) {
                    cargo = "Analista de Testes";
                } else if (resource[0].cargo === 3) {
                    cargo = "Analista de Sistemas";
                } else if (resource[0].cargo === 4) {
                    cargo = "Programador";
                } else if (resource[0].cargo === 5) {
                    cargo = "Web Designer";
                } else if (resource[0].cargo === 6) {
                    cargo = "Engenheiro de Software";
                } else if (resource[0].cargo === 7) {
                    cargo = "Professor";
                } else if (resource[0].cargo === 8) {
                    cargo = "Técnico de Informática";
                } else if (resource[0].cargo === 9) {
                    cargo = "Professor";
                } else if (resource[0].cargo === 10) {
                    cargo = "Outro";
                } else if (resource[0].cargo === 11) {
                    cargo = "Estudante";
                }

                $(".lead-cargo-upd").append('"<option value="' + resource[0].cargo + '" selected="selected">' + cargo + '</option>');

                var area_atuacao;
                if (resource[0].cargo === 1) {
                    area_atuacao = "Marketing";
                } else if (resource[0].cargo === 2) {
                    area_atuacao = "Consultorias e Treinamentos";
                } else if (resource[0].cargo === 3) {
                    area_atuacao = "Ecommerce";
                } else if (resource[0].cargo === 4) {
                    area_atuacao = "Educação e Ensino";
                } else if (resource[0].cargo === 5) {
                    area_atuacao = "Engenharia e industria Geral";
                } else if (resource[0].cargo === 6) {
                    area_atuacao = "Eventos";
                } else if (resource[0].cargo === 7) {
                    area_atuacao = "Financeiro e Jurídico e Serviços Relacionados";
                } else if (resource[0].cargo === 8) {
                    area_atuacao = "Hardware e Eletrônico";
                } else if (resource[0].cargo === 9) {
                    area_atuacao = "Imobiliárias";
                } else if (resource[0].cargo === 10) {
                    area_atuacao = "Midia";
                } else if (resource[0].cargo === 11) {
                    area_atuacao = "Ongs";
                } else if (resource[0].cargo === 12) {
                    area_atuacao = "Saude e Estética";
                } else if (resource[0].cargo === 13) {
                    area_atuacao = "Serviços em Geral";
                } else if (resource[0].cargo === 14) {
                    area_atuacao = "Serviços em RH e couch";
                } else if (resource[0].cargo === 15) {
                    area_atuacao = "Software e cloud";
                } else if (resource[0].cargo === 16) {
                    area_atuacao = "Telecomunicações";
                } else if (resource[0].cargo === 17) {
                    area_atuacao = "Turismo";
                } else if (resource[0].cargo === 18) {
                    area_atuacao = "Varejo";
                }

                $(".lead-area-atuacao-upd").append('"<option value="' + resource[0].status + '" selected="selected">' + area_atuacao + '</option>');
                ...
                $(".lead-status-upd").append('"<option value="' + resource[0].status + '" selected="selected">' + status + '</option>');
    ...
    $('#lead-update-button').click(function (event) {
        event.preventDefault();
        $.post("http://localhost/wp-content/plugins/news.core.php", $('.lead-action-update').serializeArray())
                .done(function (data) {
                    alert(data);
                    var resource = $.parseJSON(data);
